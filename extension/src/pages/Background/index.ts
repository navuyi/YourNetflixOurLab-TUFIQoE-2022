...
chrome.runtime.onInstalled.addListener(async () => {
  console.log(
    `[BackgroundScript] | ${get_local_datetime(new Date())} | Installing...`
  );
    ...
    addRules: [
      {
        ...
        action: {
          type: 'redirect',
          redirect: { extensionPath: '/cadmiumPlayercore.bundle.js' },
        },
        //@ts-ignore
        condition: {
 ...
 */
const receive_finished_signal = async (
  message: T_MESSAGE,
...
const receive_redirect_signal = async (
  message: T_MESSAGE,
  sender: chrome.runtime.MessageSender,
  sendResponse: Function
) => {
